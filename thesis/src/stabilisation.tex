...
Once the coordinates of the point $B$ are known, the principle is identical to that of the previous one: measure the slope $\alpha_2$ of the transfer function at point $B$, use the data from figure \ref{stab_ref_reflected} to calculate the standard deviation $\sigma(\epsilon^*)$ and compute the phase noise $\tind{\sigma}{ref}$ as:

\begin{equation}
	\tind{\sigma}{ref}[\si{\radian}] = \frac{\sigma(\epsilon^*)[\si{\volt}]}{\alpha_2 T_2}2\pi
\end{equation}
